...
{
	public class ThreadingLimit
	{
		public ThreadingLimit()
		{
		}
		...
		public void ProcessParallelForEachWithLimits(List<string> items) {
			...
			Parallel.ForEach(items, options, y => {
                Console.WriteLine("{0}; ThreadId: {1}", y, Thread.CurrentThread.ManagedThreadId);
            });
		...
		public bool ProcessPlinqWithLimits(List<string> items) {
			...
			return items.AsParallel().WithDegreeOfParallelism(max).Any(i => CheckString(i));
		}
